...
import { SwapOptions, SwapParams } from '../smart-contract-sdk/parameters/trade/swap'
import { MSG_SENDER, ProtocolType, SWAP_ROUTER_ADDRESS, TradeType } from '../typescript-sdk/utils/constants'
...
export abstract class SwapRouter {
  ...
  private static encodeSwap(
    ...
    for (const { route, inputAmount, outputAmount } of trade.swaps) {
      const amountIn: string = toHex(trade.maximumAmountIn(params.slippageTolerance, inputAmount).quotient)
      const amountOut: string = toHex(trade.minimumAmountOut(params.slippageTolerance, outputAmount).quotient)
      ...
      if (singleHop) {
        ...
        const path: string = addRouteToPath(route, trade.tradeType === TradeType.EXACT_OUTPUT)
  ...
  private static encodeSwaps(
    ...
    const ZERO_IN: TokenAmount<IToken> = TokenAmount.fromRawAmount(sampleTrade.inputAmount.currency, 0)
    const ZERO_OUT: TokenAmount<IToken> = TokenAmount.fromRawAmount(sampleTrade.outputAmount.currency, 0)
