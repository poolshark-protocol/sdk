...
export class RangePoolGetters  {
   ...
   private async quote(args:QuoteParams):Promise<[BigNumber, BigNumber, BigNumber]> {
        return await this.rangePool.quote(args);
   }

    private async sample(args:SampleParams):Promise<[BigNumber[], BigNumber[], BigNumber, BigNumber, number] & {
    ...
    private async snapshot(args:SnapshotParamsArgs):Promise<[BigNumber,BigNumber,BigNumber,BigNumber] & {
        secondsPerLiquidityAccum: BigNumber;
        feesOwed0: BigNumber;
        feesOwed1: BigNumber;
        tickSecondsAccum: BigNumber;
    ...
    private async balanceOf({account,id}:{account:string,id:BigNumberish}):Promise<BigNumber> {
        return await this.rangePool.balanceOf(account,id);
    }

    private async balanceOfBatch({accounts,ids}:{accounts:string[],ids:BigNumberish[]}):Promise<BigNumber[]> {
