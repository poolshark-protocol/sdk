...
export class Percent extends Ratio {
  ...
  public static from(percentString: string, precision?: number): Percent {
    ...
    const scaledValue = JSBD.multiply(
      JSBD.BigDecimal(percentString),
      JSBD.BigDecimal(scalar.toString()),
    );
    return new Percent(
      scaledValue.toString().split('.').join(''),
      JSBI.multiply(scalar, JSBI.BigInt(100)),
    );
  }
}
