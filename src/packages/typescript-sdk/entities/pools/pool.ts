...
export function computePoolAddress({
  ...
  const [token0, token1] = tokenA.sort(tokenB)
    ? [tokenA, tokenB]
...
export class Pool {
  ...
  public constructor(
    ...
    [this.token0, this.token1] = token0.sort(token1)
      ? [token0, token1]
      : [token1, token0];
    this.swapFee = fee;
    ...
    if (ticks)
      this.tickProvider = Array.isArray(ticks)
        ? new TickListDataProvider(ticks, TICK_SPACINGS[fee])
        : ticks;
    else this.tickProvider = EMPTY_TICK_DATA_PROVIDER;
  }
  ...
  public async getInputAmount(
    ...
    invariant(
      outputAmount.token.isToken && this.contains(outputAmount.token),
      'TOKEN',
    );
  ...
  private async swap(
    ...
    if (zeroForOne) {
      invariant(
        JSBI.greaterThan(sqrtPriceLimit, TickMath.MIN_SQRT_RATIO),
        'RATIO_MIN',
      );
      invariant(JSBI.lessThan(sqrtPriceLimit, this.sqrtPrice), 'RATIO_CURRENT');
    } else {
      invariant(
        JSBI.lessThan(sqrtPriceLimit, TickMath.MAX_SQRT_RATIO),
        'RATIO_MAX',
      );
      invariant(
        JSBI.greaterThan(sqrtPriceLimit, this.sqrtPrice),
        'RATIO_CURRENT',
      );
    }
    ...
    let cache: SwapCache = {
      cross: true,
      crossTick: await this.tickProvider.nextCloseTick(
        this.tickAtPrice,
        zeroForOne,
        this.tickSpacing,
      ),
