 ...
 */
export function tradeComparator<TInput extends IToken, TOutput extends IToken, TTradeType extends TradeType>(
 ...
 * @template TTradeType The trade type, either exact input or exact output
 */
export class Trade<TInput extends IToken, TOutput extends IToken, TTradeType extends TradeType> {
  /**
  ...
  public get inputAmount(): TokenAmount<TInput> {
    ...
    const totalInputFromRoutes = this.swaps
      ...
      .reduce((total, cur) => total.add(cur), TokenAmount.fromRawAmount(inputIToken, 0))
  ...
  public get outputAmount(): TokenAmount<TOutput> {
    ...
    const totalOutputFromRoutes = this.swaps
      ...
      .reduce((total, cur) => total.add(cur), TokenAmount.fromRawAmount(outputIToken, 0))
  ...
  public get priceImpact(): Percent {
    ...
    let spotOutputAmount = TokenAmount.fromRawAmount(this.outputAmount.token, 0)
   ...
   * @returns The route
   */
  public static async fromRoute<TInput extends IToken, TOutput extends IToken, TTradeType extends TradeType>(
    route: Route<TInput, TOutput>,
    amount: TTradeType extends TradeType.EXACT_INPUT ? TokenAmount<TInput> : TokenAmount<TOutput>,
    ...
    if (tradeType === TradeType.EXACT_INPUT) {
      ...
      inputAmount = TokenAmount.fromRatioAmount(route.input, amount.numerator, amount.denominator)
      ...
      inputAmount = TokenAmount.fromRatioAmount(route.input, amounts[0].numerator, amounts[0].denominator)
      outputAmount = TokenAmount.fromRatioAmount(route.output, amount.numerator, amount.denominator)
    }
   ...
   * @returns The trade
   */
  public static async fromRoutes<TInput extends IToken, TOutput extends IToken, TTradeType extends TradeType>(
    routes: {
      amount: TTradeType extends TradeType.EXACT_INPUT ? TokenAmount<TInput> : TokenAmount<TOutput>
      route: Route<TInput, TOutput>
    }[],
    ...
    for (const { route, amount } of routes) {
      ...
      if (tradeType === TradeType.EXACT_INPUT) {
        invariant(amount.token.equals(route.input), 'INPUT')
        inputAmount = TokenAmount.fromRatioAmount(route.input, amount.numerator, amount.denominator)
        amounts[0] = TokenAmount.fromRatioAmount(route.input.wrapped, amount.numerator, amount.denominator)
  ...
  private constructor({
    ...
    invariant(
      routes.every(({ route }) => inputIToken.wrapped.equals(route.input.wrapped)),
      'INPUT_CURRENCY_MATCH'
    )
    invariant(
      routes.every(({ route }) => outputIToken.wrapped.equals(route.output.wrapped)),
    ...
    for (const { route } of routes) {
      for (const pool of route.pools) {
        poolAddressSet.add(Pool.getAddress(pool.token0, pool.token1, pool.swapFee))
      }
    }
   ...
   */
  public minimumAmountOut(slippageTolerance: Percent, amountOut = this.outputAmount): TokenAmount<TOutput> {
    ...
    if (this.tradeType === TradeType.EXACT_OUTPUT) {
      ...
      return TokenAmount.fromRawAmount(amountOut.token, slippageAdjustedAmountOut)
    }
  }
   ...
   */
  public maximumAmountIn(slippageTolerance: Percent, amountIn = this.inputAmount): TokenAmount<TInput> {
    invariant(!slippageTolerance.lessThan(ZERO), 'SLIPPAGE_TOLERANCE')
    if (this.tradeType === TradeType.EXACT_INPUT) {
      return amountIn
    } else {
      const slippageAdjustedAmountIn = new Ratio(ONE).add(slippageTolerance).multiply(amountIn.quotient).quotient
      return TokenAmount.fromRawAmount(amountIn.token, slippageAdjustedAmountIn)
    }
  }
   ...
   * @returns The exact in trade
   */
  public static async bestTradeExactIn<TInput extends IToken, TOutput extends IToken>(
    pools: Pool[],
      ...
      if (amountOut.token.isToken && amountOut.token.equals(tokenOut)) {
        sortedInsert(
          ...
          await Trade.fromRoute(
            new Route([...currentPools, pool], currencyAmountIn.token, currencyOut),
            currencyAmountIn,
   ...
   * @returns The exact out trade
   */
  public static async bestTradeExactOut<TInput extends IToken, TOutput extends IToken>(
    pools: Pool[],
    currencyIn: TInput,
    ...
    for (let i = 0; i < pools.length; i++) {
      ...
      if (amountIn.token.equals(tokenIn)) {
        sortedInsert(
          bestTrades,
          await Trade.fromRoute(
            new Route([pool, ...currentPools], currencyIn, currencyAmountOut.token),
            currencyAmountOut,
