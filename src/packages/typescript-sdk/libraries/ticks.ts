...
export class Ticks {
  ...
  private static _quoteSingle(
    ...
    if (
      zeroForOne ? priceLimit >= cache.sqrtPrice : priceLimit <= cache.sqrtPrice
    ) {
      cache.cross = false;
      return cache;
    }
    ...
    if (zeroForOne) {
      ...
      let maxDx = ConstantProduct.getDx(
        cache.liquidity,
        nextPrice,
        cache.sqrtPrice,
        true,
      ...
      if (cache.amountInLeft <= maxDx) {
        let newPrice = ConstantProduct.getNextPrice(
          cache.sqrtPrice,
          nextPrice,
          cache.liquidity,
          cache.amountInLeft,
      ...
      let maxDy = ConstantProduct.getDy(
        cache.liquidity,
        cache.sqrtPrice,
        nextPrice,
        true,
  ...
  private static async _cross(
    ...
    const liquidityDelta = JSBI.BigInt(
      (
        await pool.tickProvider.getTick(cache.crossTick)
      ).liquidityDelta.toString(),
    );
    if (zeroForOne) {
      cache.liquidity = JSBI.subtract(cache.liquidity, liquidityDelta);
      cache.tick = cache.crossTick;
      cache.crossTick = await pool.tickProvider.nextCloseTick(
        cache.tick,
        zeroForOne,
        pool.tickSpacing,
      );
