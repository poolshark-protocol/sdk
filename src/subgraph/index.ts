 ...
 */
   async getRangePoolFromFactory(tokenA: `0x${string}`, tokenB: `0x${string}`, feeTierId?: number):Promise<GetRangePoolFromFactoryResponseFeeTierIdUndefined | GetRangePoolFromFactoryResponseFeeTierIdDefined> {
    const token0 = tokenA.localeCompare(tokenB) < 0 ? tokenA : tokenB
    const token1 = tokenA.localeCompare(tokenB) < 0 ? tokenB : tokenA
    
      const getPoolQuery = feeTierId == undefined ? 
          `
          {
  ...
  async fetchCoverPositions(address: string,skip?:number,limit?:number):Promise<FetchCoverPositionsResponse> {
...
async fetchCoverPools (skip?:number,limit?:number):Promise<FetchCoverPoolsResponse> {
    const poolsQuery = `
            query($id: String) {
...
async fetchCoverPoolMetrics (limit?:number,offset?:number):Promise<FetchCoverPoolMetricsResponse> {
...
async  fetchRangePools (skip?:number,limit?:number):Promise<FetchRangePoolsResponse> {
    const poolsQuery = `
            query($id: String) {
...
async fetchRangePositions  (address: string,skip?:number,limit?:number):Promise<FetchRangePositionsResponse> {
    const positionsQuery = `
    {
      positionFractions(where: {owner:"${address}"},skip:${skip || 0},first:${limit || 100} ) {
        id
        owner
        amount
        token{
...
async fetchRangeMetrics (skip?:number,limit?:number):Promise<FetchRangePoolMetricsResponse> {
    const positionsQuery = `
        query($id: String) {
            rangePoolFactories(id: $id,skip:${skip || 0},first:${limit || 100}) {
                id
                txnCount
                feesEthTotal
                feesUsdTotal
...
async fetchUniV3Positions(address: string,skip?:number,limit?:number):Promise<FetchUniV3PositionsResponse> {
    const univ3PositionsQuery = `
            query($owner: String) {
                positions(owner: $owner,skip:${skip || 0},first:${limit || 100}) {
                    id
                    liquidity
                    owner
                    token1{
...
async fetchPrice (address: string,skip?:number,limit?:number):Promise<FetchPriceResponse> {
    const univ3Price = `
            {
